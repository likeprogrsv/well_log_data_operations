...
sheet2_columns = ['oil field', 'well', 'UWI', 'X', 'Y', 'depth top', 'depth bot', 'T', 'AO', 'GK mean', 'NKT mean', 'IK mean', \
    'S1+S2 mean', 'TOC mean']
...
param = {
            ...
            'well_name': 'Скважина',
            'x_coord': 'X на подошву',
            'y_coord': 'Y на подошву',
            'T': 'Т',
            'AO': 'АО',
...
def curr_well_excel_data(las_uwi):
    ...
    data = excel_df.loc[excel_df['UWI'] == las_uwi, (param['oil_field'], param['well_name'], param['T'], param['x_coord'], \
       param['y_coord'], param['depth_top'], param['depth_bot'], param['AO'], param['abs_top'] , param['abs_bot'])]
    if data.empty:
        ...
        for filename in os.scandir(project_path + '\\' +las_files_path):
            if filename.is_file():         
                ...
                try:
                    ...
                    # Записываем ошибки
                    errors_df = pd.DataFrame({'Скв отсутст в экселе': '-', 'Скв с отриц знач в кривых': '-', \
                        'Не правильная форма данных кривых': filename.name}, index=[writer.sheets[errors_sheet3_name].max_row])
                ...
                try:
                    oil_field, well_name, t, x_coord, y_coord, depth_t, depth_b, ao, abs_t, abs_b = (float(well_excel[i].to_numpy()) \
                        if well_excel[i].dtype != 'object' else well_excel[i].values.astype(str)[0] for i in well_excel)
                except TypeError:                            
                    # Записываем ошибки
                    errors_df = pd.DataFrame({'Скв отсутст в экселе': filename.name, 'Скв с отриц знач в кривых': '-', \
                        'Не правильная форма данных кривых': '-'}, index=[writer.sheets[errors_sheet3_name].max_row])
                    write_results(writer, errors_sheet3_name, errors_df)
                    continue
                ...
                row_for_sheet2 = pd.DataFrame([oil_field, well_name, las_uwi, x_coord, y_coord, depth_t, depth_b, t, ao, sheet1_df['GK'].mean(), \
